...
- **v0.1.20**：翻译质量核心升级 —— **批处理翻译**：连续段落打包（`[B0]/[B1]…`）一次调用，模型获得上下文，大幅减少碎片幻觉（此前逐块翻译，残片输入诱发模型编造整段无关内容）；**参考文献条目（[N] 开头）与公式/符号残片不再送 LLM**，直接保留原文，从源头杜绝幻觉与"无法理解"类废话；系统提示词重构（弱化领域暗示、新增"禁止编造/禁止解释"硬约束，参考用户提供的 HY3 提示词方案）。已用 Kimi API 实测：批处理格式严格遵守、参考文献/残片跳过、公式+说明正确翻译。
