...
- **v0.1.24**：段落分明重构 —— 断段规则改为**首行缩进/行距/换栏/列表项**判断（不再按行尾标点断段，英文排版每行常在句号处结束，旧规则把同一自然段拆成碎片）；双栏检测改为**行级**（按 y 聚合 + 最大间隙/`[N]` 编号信号拆分左右栏，行宽判断跨栏行），修复混合布局页（跨栏标题+单栏摘要+双栏正文）分栏线被拉偏导致的段落交错；修复页眉行参与行距计算导致的误断段。
- **v0.1.23**："干净优先"策略 —— 公式行/数字密集残片（含公式编号 `(1.1)` 等）一律不进 LLM、原样保留，杜绝"中文夹杂 gument/rec 等杂乱字符"；译文混入公式符号（≥2 个）自动回退原文；空翻译结果回退原文；渲染端跳过空段落。
- **v0.1.22**：翻译质量再优化 —— 表格行（TABLE）不再送 LLM 直接保留原文（模型会把表格当文本乱翻）；章节编号（I./II./1.）强制保留原文形式（不再出现"第三部分"）；`stripMarker` 清理全文残留的中英文结构标记（含模型翻译出的 `[段落]`/`[表格]`/`[图]`）；批大小 4→8（模型上下文更大）；去重保护（LLM 偶发重复输出时后一块恢复原文）。
- **v0.1.21**：修复批处理回退崩溃 —— 批量请求失败回退逐块翻译时索引错误（`b.idx` 不存在）导致 `can't access property 'blk'` 崩溃。已修正并用两个场景实测：批量+逐块全失败时保留原文并记录错误（不再崩溃）、temperature 降级路径正常。
- **v0.1.20**：翻译质量核心升级 —— **批处理翻译**：连续段落打包（`[B0]/[B1]…`）一次调用，模型获得上下文，大幅减少碎片幻觉（此前逐块翻译，残片输入诱发模型编造整段无关内容）；**参考文献条目（[N] 开头）与公式/符号残片不再送 LLM**，直接保留原文，从源头杜绝幻觉与"无法理解"类废话；系统提示词重构（弱化领域暗示、新增"禁止编造/禁止解释"硬约束，参考用户提供的 HY3 提示词方案）。
- **v0.1.19**：翻译质量再优化 —— 清理模型输出残留（标题的 `##` 前缀、任意结构标记 `[TABLE]` 等统一剥离）；提示词强化（标题直接输出不加 `#`、参考文献条目必须 100% 保留英文原文）；抽取侧过滤 arXiv 页眉与页码行、图注与描述文字合并、参考文献条目按 `[N]` 聚合不再被拆段；空表格降级为等宽文本显示。
- **v0.1.18**：修复推理模型（如 Kimi k2 系列）翻译全部失败 —— 这类模型只允许 `temperature=1`，插件默认 0.2 导致每次请求 400（`invalid temperature`），全部段落保留原文。现在检测到 temperature 报错会自动去掉该参数重试；失败提示中附带具体错误信息便于排查。
